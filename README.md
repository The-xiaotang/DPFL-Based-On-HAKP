...
$PSS_i = \frac{F_i}{T} \times (1 - CV_{interval})$
其中 $F_i$ 是客户端 $i$ 在过去 $T$ 轮中的参与频率（次数）。 $CV_{interval}$ 是参与时间间隔的变异系数（标准差 / 均值）。较低的 $CV$ 表示参与模式更具可预测性和规律性。
...
*   **升级至核心层:** $CVS_i > \theta_{high} + \delta$
*   **降级至边缘层:** $CVS_i < \theta_{low} - \delta$
...
4.  **本地训练与上传 (Local Training & Upload):** 活跃客户端 $C_t$ 执行本地训练。核心和活跃客户端上传完整的模型更新 $\theta_i$；边缘客户端计算并上传知识原型 $p_i$。
